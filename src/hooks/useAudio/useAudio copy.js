...
export default function useAudio({ contextName, waveformRef = "default", initialVolume = 1, autoReset = false, onEnded = null }) {
    ...
    const generateTimelineGraph = async (tempo) => {
        ...
        const bpmPointsData = await generateBeatPattern(bufferRef.current.duration, newTempo, newOffset);
    ...
    const startTimeUpdate = () => {
        ...
        const updateTime = () => {
            const newTime = (pausedAtRef.current + (audioContext.currentTime - startTimeRef.current)) * playbackRateRef.current;
    ...
    const play = () => {
        ...
        try {
            ...
            sourceRef.current.playbackRate.setValueAtTime(playbackRateRef.current, audioContext.currentTime); // Update playback rate dynamically
    ...
    const seek = (position) => {
        ...
        try {
            ...
            const seekTime = typeof position === 'number'
                ? position
